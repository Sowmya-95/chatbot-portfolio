<script lang="ts">
  import { onMount } from 'svelte';
  import { gsap } from 'gsap';
  import { createMagneticHover, createRipple, springConfig } from '$lib/utils/animations';
  import { profile } from '$lib/data/profile';

  let container: HTMLDivElement;

  onMount(() => {
    // Spring entrance for the whole container
    gsap.from(container, {
      y: 30,
      opacity: 0,
      duration: 0.6,
      ease: springConfig.bouncy
    });

    // Stagger entrance for buttons
    const btns = container.querySelectorAll('.contact-btn');
    gsap.fromTo(btns,
      { y: 20, opacity: 0, scale: 0.9 },
      { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.1, delay: 0.2, ease: springConfig.bouncy }
    );

    // Magnetic hover on each button
    btns.forEach((btn) => {
      createMagneticHover(btn as HTMLElement, 0.2);
    });
  });
</script>

<div class="contact-container" bind:this={container}>
  <h3 class="contact-heading">Let's connect!</h3>

  <div class="buttons-row">
    <a
      class="contact-btn"
      href="mailto:{profile.links.email}"
      target="_blank"
      rel="noopener noreferrer"
      onclick={(e) => createRipple(e, e.currentTarget)}
    >
      <span class="btn-icon">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
      </span>
      <span class="btn-label">Email</span>
    </a>

    <a
      class="contact-btn"
      href={profile.links.github}
      target="_blank"
      rel="noopener noreferrer"
      onclick={(e) => createRipple(e, e.currentTarget)}
    >
      <span class="btn-icon">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/></svg>
      </span>
      <span class="btn-label">GitHub</span>
    </a>

    <a
      class="contact-btn"
      href={profile.links.linkedin}
      target="_blank"
      rel="noopener noreferrer"
      onclick={(e) => createRipple(e, e.currentTarget)}
    >
      <span class="btn-icon">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
      </span>
      <span class="btn-label">LinkedIn</span>
    </a>
  </div>
</div>

<style>
  .contact-container {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 1rem;
    padding: 1rem 0;
  }

  .contact-heading {
    margin: 0;
    font-family: 'JetBrains Mono', monospace;
    font-size: 1.1rem;
    font-weight: 700;
    color: rgb(0, 255, 180);
    text-align: center;
  }

  .buttons-row {
    display: flex;
    gap: 0.75rem;
    flex-wrap: wrap;
    justify-content: center;
  }

  .contact-btn {
    position: relative;
    overflow: hidden;
    display: flex;
    align-items: center;
    gap: 0.5rem;
    padding: 10px 20px;
    border-radius: 6px;
    background: rgba(0, 255, 180, 0.1);
    border: 1px solid rgba(0, 255, 180, 0.4);
    color: rgb(0, 255, 180);
    text-decoration: none;
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.85rem;
    font-weight: 500;
    transition: all 0.25s ease;
    cursor: pointer;
  }

  .contact-btn:hover {
    transform: scale(1.08);
    background: rgba(0, 255, 180, 0.1);
    border-color: rgba(0, 255, 180, 0.5);
    box-shadow: 0 0 20px rgba(0, 255, 180, 0.18);
  }

  .contact-btn:active {
    transform: scale(0.95);
  }

  .btn-icon {
    display: flex;
    align-items: center;
    justify-content: center;
  }

  .btn-label {
    white-space: nowrap;
  }
</style>
